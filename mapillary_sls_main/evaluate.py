...
from mapillary_sls_main.mapillary_sls.utils.eval import eval, create_dummy_predictions, download_msls_sample
...
def main():
    ...
    if f:
        f.close()
    return line, metrics['{}@{}'.format('recall', 1)]

def eval_api(predictions, ks = [1, 5, 10, 20], root_default = ''):

    threshold = 25
    cities = '' # (sf,cph)
    task = 'im2im'    # '[im2im, seq2im, im2seq, seq2seq]'
    # parser.add_argument('--seq-length',
    #                     type=int,
    #                     default=3,
    #                     help='Sequence length to evaluate on for seq2X and X2seq tasks')
    seq_length = 3
    subtask = 'all'  # Subtask to evaluate on: [all, s2w, w2s, o2n, n2o, d2n, n2d]
    output = None
    # select for which ks to evaluate
    ...
    for metric in ['recall', 'map']:
        for k in ks:
            line =  '{}_{}@{}: {:.3f}'.format(subtask,
                                              ...
                                              k,
                                              metrics['{}@{}'.format(metric, k)])
            recalls.append(metrics['{}@{}'.format('recall', k)])
            print(line)
            logging.debug(line)
            if f:
                f.write(line + '\n')
